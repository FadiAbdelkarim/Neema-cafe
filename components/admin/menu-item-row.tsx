    ...
    <div className="px-4 sm:px-6 py-6">
      <div className="flex items-center gap-4 mb-4">
        {item.image_url && (
          ...
          <img src={item.image_url} alt={item.name} className="w-20 h-20 shrink-0 object-cover rounded-sm" />
        )}
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
            <span className="text-ink font-medium text-lg">{item.name}</span>
            {item.name_ar && (
              <span dir="rtl" className="font-arabic text-ink/60 text-lg">{item.name_ar}</span>
            )}
          </div>
          {!item.is_available && (
            <span className="inline-block mt-1 text-[10px] uppercase tracking-wider bg-terracotta/15 text-terracotta px-2 py-0.5 rounded-sm">
              Sold out
            </span>
          )}
        </div>
